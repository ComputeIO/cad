...
void CLAYER_TRIANGLES::WriteGLTF( const std::string& gltf_name ) const {
    wxString gltf_dirname = wxString( gltf_name );

    if( wxFileName::Exists( gltf_dirname ) ) {
        // TODO
    } else {
        wxDir::Make( gltf_dirname );
    }

    wxFileName gltf_filename = wxFileName();
    gltf_filename.Assign (
            gltf_dirname,
            gltf_dirname,
            wxString::FromUTF8Unchecked( "gltf" )
    );

    OPEN_OSTREAM( op, gltf_filename.GetFullPath() );
    op.imbue( std::locale( "C" ) );

    int top_length = m_layer_top_triangles->GetVertexSize();
    int top_bytes = top_length * sizeof(SFVEC3F);

    int mid_length = m_layer_middle_contourns_quads->GetVertexSize();
    int mid_bytes = mid_length * sizeof(SFVEC3F);

    int top_seg_length = m_layer_top_segment_ends->GetVertexSize();
    int top_seg_bytes = top_seg_length * sizeof(SFVEC3F);

    SFVEC2F *top_seg_uv = new SFVEC2F[top_seg_length];

    for( int i = 0; i < top_seg_length; i += 3 ) {
        top_seg_uv[i + 0] = SFVEC2F( 1.0f, 0.0f );
        top_seg_uv[i + 1] = SFVEC2F( 0.0f, 1.0f );
        top_seg_uv[i + 2] = SFVEC2F( 0.0f, 0.0f );
    }
    ...
    float mid_min_y = -5.0;
    float mid_min_z = 0.0;
    float mid_max_x = 11.0;
    float mid_max_y = -1.0;
    float mid_max_z = 1.0;
    // TODO
    float top_seg_min_x = 5.0;
    float top_seg_min_y = -5.0;
    float top_seg_min_z = 0.0;
    float top_seg_max_x = 11.0;
    float top_seg_max_y = -1.0;
    float top_seg_max_z = 1.0;

    op << R"({)";
    op << R"(  "scenes":[{"nodes":[0,1,2]}],)";
    op << R"(  "nodes":[)";
    op << R"(    {)";
    op << R"(      "mesh":0)";
    op << R"(    },)";
    ...
    op << R"(  "buffers":[)";
    op << R"(    {)";
    op << R"(      "uri":")" << gltf_name << R"(.bin",)";
    op << R"(      "byteLength":)" << top_bytes + mid_bytes + mid_bytes + top_seg_bytes + top_seg_uv_bytes;
    op << R"(    })";
    op << R"(  ],)";
    op << R"(  "bufferViews":[)";
    op << R"(    {)";
    ...
    op << R"(      "buffer":0,)";
    op << R"(      "byteOffset":0,)";
    op << R"(      "byteLength":)" << top_bytes << R"(,)";
    op << R"(      "target":34962)";
    op << R"(    },)";
    op << R"(    {)";
    //              mid vertices
    op << R"(      "buffer":0,)";
    op << R"(      "byteOffset":)" << top_bytes << R"(,)";
    op << R"(      "byteLength":)" << mid_bytes << R"(,)";
    op << R"(      "target":34962)";
    op << R"(    },)";
    op << R"(    {)";
    //              mid normals
    op << R"(      "buffer":0,)";
    op << R"(      "byteOffset":)" << top_bytes + mid_bytes << R"(,)";
    op << R"(      "byteLength":)" << mid_bytes << R"(,)";
    op << R"(      "target":34962)";
    op << R"(    },)";
    op << R"(    {)";
    //              top segment vertices
    op << R"(      "buffer":0,)";
    op << R"(      "byteOffset":)" << top_bytes + mid_bytes + mid_bytes << R"(,)";
    op << R"(      "byteLength":)" << top_seg_bytes << R"(,)";
    op << R"(      "target":34962)";
    op << R"(    },)";
    op << R"(    {)";
    //              top segment uv
    op << R"(      "buffer":0,)";
    op << R"(      "byteOffset":)" << top_bytes + mid_bytes + mid_bytes + top_seg_bytes << R"(,)";
    op << R"(      "byteLength":)" << top_seg_uv_bytes << R"(,)";
    op << R"(      "target":34962)";
    op << R"(    })";
    op << R"(  ],)";
    op << R"(  "accessors":[)";
    ...
    op << R"(      "bufferView":0,)";
    op << R"(      "byteOffset":0,)";
    op << R"(      "componentType":5126,)";
    op << R"(      "count":)" << top_length << R"(,)";
    op << R"(      "type":"VEC3",)";
    op << R"(      "max":[)" << top_max_x << "," << top_max_y << "," << top_max_z << R"(],)";
    op << R"(      "min":[)" << top_min_x << "," << top_min_y << "," << top_min_z << R"(])";
    op << R"(    },)";
    op << R"(    {)";
    op << R"(      "bufferView":1,)";
    op << R"(      "byteOffset":0,)";
    op << R"(      "componentType":5126,)";
    op << R"(      "count":)" << mid_length << R"(,)";
    op << R"(      "type":"VEC3",)";
    op << R"(      "max":[)" << mid_max_x << "," << mid_max_y << "," << mid_max_z << R"(],)";
    op << R"(      "min":[)" << mid_min_x << "," << mid_min_y << "," << mid_min_z << R"(])";
    op << R"(    },)";
    op << R"(    {)";
    op << R"(      "bufferView":2,)";
    op << R"(      "byteOffset":0,)";
    ...
    op << R"(      "byteOffset":0,)";
    op << R"(      "componentType":5126,)";
    op << R"(      "count":)" << top_seg_length << R"(,)";
    op << R"(      "type":"VEC3",)";
    op << R"(      "max":[)" << top_seg_max_x << "," << top_seg_max_y << "," << top_seg_max_z << R"(],)";
    op << R"(      "min":[)" << top_seg_min_x << "," << top_seg_min_y << "," << top_seg_min_z << R"(])";
    op << R"(    },)";
    op << R"(    {)";
    op << R"(      "bufferView":4,)";
    op << R"(      "byteOffset":0,)";
    op << R"(      "componentType":5126,)";
    op << R"(      "count":)" << top_seg_uv_length << R"(,)";
    op << R"(      "type":"VEC2",)";
    op << R"(      "max":[)" << 1. << "," << 1. << R"(],)";
    op << R"(      "min":[)" << 0. << "," << 0. << R"(])";
    op << R"(    })";
    ...
    op << R"(  "materials":[)";
    op << R"(    {)";
    op << R"(      "pbrMetallicRoughness":{)";
    op << R"(        "baseColorFactor":[1.0,1.0,1.0,0.9])";
    op << R"(        "baseColorTexture":{)";
    op << R"(          "index":0,)";
    op << R"(          "texCoord":0)";
    op << R"(        })";
    op << R"(      },)";
    op << R"(      "alphaMode":"BLEND")";
    op << R"(    })";
    op << R"(  ],)";
    op << R"(  "textures":[)";
    op << R"(    {)";
    ...
    op << R"(  "images":[)";
    op << R"(    {)";
    op << R"(      "uri":"circleImage_blured.png.png")";
    op << R"(    })";
    op << R"(  ],)";
    op << R"(  "samplers":[)";
    op << R"(    {)";
    ...
    wxFileName bin_filename = wxFileName();
    bin_filename.Assign (
            gltf_dirname,
            gltf_dirname,
            wxString::FromUTF8Unchecked( "bin" )
    );

    wxFile buffer_file;
    buffer_file.Create( bin_filename.GetFullPath(), true );

    buffer_file.Write(m_layer_top_triangles->GetVertexPointer(), top_bytes);
    buffer_file.Write(m_layer_middle_contourns_quads->GetVertexPointer(), mid_bytes);
    buffer_file.Write(m_layer_middle_contourns_quads->GetNormalsPointer(), mid_bytes);
    buffer_file.Write(m_layer_top_segment_ends->GetVertexPointer(), top_seg_bytes);
    buffer_file.Write(top_seg_uv, top_seg_uv_bytes);
}
