...
ERC_ITEM ERC_ITEM::pinNotConnected( ERCE_PIN_NOT_CONNECTED, _( "Pin not connected" ),
...
ERC_ITEM ERC_ITEM::pinTableWarning( ERCE_PIN_TO_PIN_WARNING, _( "Conflict problem between pins" ),
                                    wxT( "pin_to_pin" ) );

ERC_ITEM ERC_ITEM::pinTableError( ERCE_PIN_TO_PIN_ERROR, _( "Conflict problem between pins" ),
                                  wxT( "pin_to_pin" ) );
...
ERC_ITEM ERC_ITEM::labelDangling( ERCE_LABEL_NOT_CONNECTED, _( "Label not connected to anything" ),
                                  wxT( "label_dangling" ) );

ERC_ITEM ERC_ITEM::globalLabelDangling(
        ERCE_GLOBLABEL, _( "Global label not connected anywhere else in the schematic" ),
...
ERC_ITEM ERC_ITEM::differentUnitFootprint(
        ERCE_DIFFERENT_UNIT_FP, _( "Different footprint assigned in another unit of the symbol" ),
...
ERC_ITEM ERC_ITEM::netNotBusMember(
        ERCE_BUS_ENTRY_CONFLICT, _( "Net is graphically connected to a bus but not a bus member" ),
...
ERC_ITEM
ERC_ITEM::busToBusConflict( ERCE_BUS_TO_BUS_CONFLICT,
...
ERC_ITEM ERC_ITEM::unresolvedVariable( ERCE_UNRESOLVED_VARIABLE, _( "Unresolved text variable" ),
                                       wxT( "unresolved_variable" ) );

ERC_ITEM ERC_ITEM::wireDangling( ERCE_WIRE_DANGLING, _( "Wires not connected to anything" ),
                                 wxT( "wire_dangling" ) );

ERC_ITEM ERC_ITEM::libSymbolIssues( ERCE_LIB_SYMBOL_ISSUES, _( "Library symbol issue" ),
                                    wxT( "lib_symbol_issues" ) );

ERC_ITEM ERC_ITEM::unannotated( ERCE_UNANNOTATED, _( "Symbol is not annotated" ),
                                wxT( "unannotated" ) );

ERC_ITEM ERC_ITEM::extraUnits( ERCE_EXTRA_UNITS, _( "Symbol has more units than are defined" ),
                               wxT( "extra_units" ) );
...
std::shared_ptr<ERC_ITEM> ERC_ITEM::Create( int aErrorCode )
{
    ...
    default: wxFAIL_MSG( "Unknown ERC error code" ); return nullptr;
    }
