...
void lut_dump(uint size)
{
    ...
    std::cout<<"Summary for 2R, 3R, 4R:"<<std::endl;
    for (i=0; i<size;  i++)
    {
        std::cout<<\
        rslt_lut[i].e_use<<" "<<\
    ...
    {                                           // go through all 2R solutions 
        for (j=0; j<size;  j++)			// combine all with itself
        {
            ...
            {                                                           // then take it
                ...
                s.append ( " )" );
                rslt_lut[S4R].e_name = s;	                        // save the result and
                rslt_lut[S4R].e_use = true;                             // enable for later use
    ...
    {                                           // go through all 2R solutions 
        for (j=0; j<size;  j++)			// combine all with itself
        {
            ...
            {                                                           // then take it
                ...
                s.append ( comb_lut[j].e_name );                        // with 2nd 2 components
                s.append ( " )" );
                rslt_lut[S4R].e_name = s;				// save the result
...
void combine3(uint size)
{
    ...
    {                                           // go through all values from used E-serie 
        if ( act_serie[i].e_use)                // skip all excluded values
        {
          for (j=0; j<size;  j++)               // combine with all 2R combinations
            {                                   // calculate R+2R serial combi
                tmp_result = comb_lut[j].e_value + act_serie[i].e_value;    
    ...
    {                                           // go through all values from used E-serie 
        ...
        {
            for (j=0; j<size;  j++)             // combine with all 2R combinations
            {                                   // this is calculation of parallel combination
                tmp_result = 1/( 1/comb_lut[j].e_value + 1/act_serie[i].e_value ); 
                tmp_result -= targetR;                                      // calculate deviation
...
void PCB_CALCULATOR_FRAME::OnCalculateESeries( wxCommandEvent& event )
{
    ...
    targetR *= 1000;       // convert to KiloOhms
    #ifdef DEBUG
