...
static DESIGN_BLOCK_LIST* get_instance_from_id( KIWAY& aKiway, int aId )
{
    void* ptr = nullptr;
    ...
    try
    {
        ...
        return static_cast<DESIGN_BLOCK_LIST*>( ptr );
    }
    catch( ... )
    {
        return nullptr;
...
DESIGN_BLOCK_LIST* DESIGN_BLOCK_LIST::GetInstance( KIWAY& aKiway )
{
    //DESIGN_BLOCK_LIST* designBlockInfo = get_instance_from_id( aKiway, KIFACE_DESIGN_BLOCK_LIST );

    //if( !designBlockInfo )
        return nullptr;

    //if( !designBlockInfo->GetCount() )
        //designBlockInfo->ReadCacheFromFile( aKiway.Prj().GetProjectPath() + wxS( "design-block-info-cache" ) );

    //return designBlockInfo;
}
