...
std::string NODE::typeString() const
{
    ...
    if( is<MARKUP::subscript>() )
        os << "SUBSCRIPT";
    else if( is<MARKUP::superscript>() )
        os << "SUPERSCRIPT";
    else if( is<MARKUP::anyString>() )
        os << "ANYSTRING";
    else if( is<MARKUP::anyStringWithinBraces>() )
        os << "ANYSTRINGWITHINBRACES";
    else if( is<MARKUP::dollar>() )
        os << "DOLLAR";
    else if( is<MARKUP::tilde>() )
        os << "TILDE";
    else if( is<MARKUP::varNamespaceName>() )
        os << "VARNAMESPACENAME";
    else if( is<MARKUP::varName>() )
        os << "VARNAME";
    else if( is<MARKUP::tildeString>() )
        os << "TILDESTRING";
    else
        os << "other";
...
{
    return is<MARKUP::subscript>();
}

bool NODE::isSuperscript() const
{
    return is<MARKUP::superscript>();
}
