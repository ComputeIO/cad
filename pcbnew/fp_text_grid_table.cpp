        ...
        m_userUnits( aUserUnits ), m_frame( aFrame )
{
    ...
    {
        g_menuOrientations.push_back( wxT( "0 " )
                                      + GetAbbreviatedUnitsLabel( EDA_UNITS::DEGREES ) );
        g_menuOrientations.push_back( wxT( "90 " )
                                      + GetAbbreviatedUnitsLabel( EDA_UNITS::DEGREES ) );
        g_menuOrientations.push_back( wxT( "-90 " )
                                      + GetAbbreviatedUnitsLabel( EDA_UNITS::DEGREES ) );
        g_menuOrientations.push_back( wxT( "180 " )
                                      + GetAbbreviatedUnitsLabel( EDA_UNITS::DEGREES ) );
    }
...
bool FP_TEXT_GRID_TABLE::CanGetValueAs( int aRow, int aCol, const wxString& aTypeName )
{
    ...
    case FPT_XOFFSET:
    case FPT_YOFFSET: return aTypeName == wxGRID_VALUE_STRING;

    case FPT_SHOWN:
    case FPT_ITALIC:
    case FPT_UPRIGHT: return aTypeName == wxGRID_VALUE_BOOL;

    case FPT_LAYER: return aTypeName == wxGRID_VALUE_NUMBER;
...
wxGridCellAttr* FP_TEXT_GRID_TABLE::GetAttr( int aRow, int aCol, wxGridCellAttr::wxAttrKind )
{
    ...
    case FPT_THICKNESS:
    case FPT_XOFFSET:
    case FPT_YOFFSET: return nullptr;

    case FPT_SHOWN:
    case FPT_ITALIC:
    case FPT_UPRIGHT: m_boolColAttr->IncRef(); return m_boolColAttr;

    case FPT_LAYER: m_layerColAttr->IncRef(); return m_layerColAttr;

    case FPT_ORIENTATION: m_orientationColAttr->IncRef(); return m_orientationColAttr;

    default: wxFAIL; return nullptr;
    }
}
...
wxString FP_TEXT_GRID_TABLE::GetValue( int aRow, int aCol )
{
    ...
    switch( aCol )
    {
    case FPT_TEXT: return text.GetText();

    case FPT_WIDTH: return StringFromValue( m_userUnits, text.GetTextWidth() );

    case FPT_HEIGHT: return StringFromValue( m_userUnits, text.GetTextHeight() );

    case FPT_THICKNESS: return StringFromValue( m_userUnits, text.GetTextThickness() );

    case FPT_LAYER: return text.GetLayerName();

    case FPT_ORIENTATION:
        return StringFromValue( EDA_UNITS::DEGREES, (int) NormalizeAnglePos( text.GetTextAngle() ),
                                true );

    case FPT_XOFFSET: return StringFromValue( m_userUnits, text.GetPos0().x );

    case FPT_YOFFSET: return StringFromValue( m_userUnits, text.GetPos0().y );
...
void FP_TEXT_GRID_TABLE::SetValue( int aRow, int aCol, const wxString& aValue )
{
    ...
    switch( aCol )
    {
    case FPT_TEXT: text.SetText( aValue ); break;

    case FPT_WIDTH: text.SetTextWidth( ValueFromString( m_userUnits, aValue ) ); break;

    case FPT_HEIGHT: text.SetTextHeight( ValueFromString( m_userUnits, aValue ) ); break;

    case FPT_THICKNESS: text.SetTextThickness( ValueFromString( m_userUnits, aValue ) ); break;
...
void FP_TEXT_GRID_TABLE::SetValueAsBool( int aRow, int aCol, bool aValue )
{
    ...
    switch( aCol )
    {
    case FPT_SHOWN: text.SetVisible( aValue ); break;

    case FPT_ITALIC: text.SetItalic( aValue ); break;

    case FPT_UPRIGHT: text.SetKeepUpright( aValue ); break;
