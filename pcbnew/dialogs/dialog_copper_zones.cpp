...
private:
    using NET_FILTERS = std::vector<std::unique_ptr<EDA_PATTERN_MATCH>>;
...
void DIALOG_COPPER_ZONE::buildListOfNets( const NETINFO_LIST& nets )
{
    ...
    {
        const auto& netCode = net->GetNetCode();
        const auto& netName = getUnescapedNetName( net );
...
{
    const auto netSelection{ m_ListNetNameSelection->GetSelection() };

    if( netSelection )
    {
        const auto& selectedNetName = m_ListNetNameSelection->GetString( netSelection );
...
void DIALOG_COPPER_ZONE::updateDisplayedListOfNets()
{
    ...
    auto listOfNets = buildListOfNetsToDisplay();

    const auto selectedNet = ensureSelectedNetIsVisible( m_currentlySelectedNetcode, listOfNets );
...
{
    auto netNameShowFilter = m_ShowNetNameFilter->GetValue();
...
wxArrayString DIALOG_COPPER_ZONE::buildListOfNetsToDisplay()
{
    ...
    const auto sortedNetNames = getSortedNetNamesList();

    const auto netsAfterShowFilter = applyShowFilter( sortedNetNames );

    auto filteredNetNames = applyHideFilterIfRequired( netsAfterShowFilter );
...
wxArrayString DIALOG_COPPER_ZONE::applyShowFilter( const wxArrayString& netNames )
{
    ...
    {
        for( const auto& filter : m_showNetsFilter )
        {
...
int DIALOG_COPPER_ZONE::ensureSelectedNetIsVisible( int selectedNetCode, wxArrayString& netsList )
{
    ...
    if( selectedNetCode > INVALID_NET_CODE )
    {
        ...
        if( selectedNet )
        {
            const auto& netName = getUnescapedNetName( selectedNet );
...
void DIALOG_COPPER_ZONE::updateInfoBar()
{
    if( m_currentlySelectedNetcode <= INVALID_NET_CODE && !m_copperZoneInfo->IsShown() )
    {
        m_copperZoneInfo->ShowMessage(
                _T( "Selecting <no net> will create an isolated copper island." ), wxICON_WARNING );
    }
