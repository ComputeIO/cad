        ...
        if( m_plotter->GetPlotterType() == PLOT_FORMAT::GERBER )
        {
            static_cast<GERBER_PLOTTER*>( m_plotter )
                    ->FlashPadChamferRoundRect(
                            shape_pos, aPad->GetSize(), aPad->GetRoundRectCornerRadius(),
                            aPad->GetChamferRectRatio(), aPad->GetChamferPositions(),
                            aPad->GetOrientation(), aPlotMode, &gbr_metadata );
            break;
        }
        KI_FALLTHROUGH;
...
void BRDITEMS_PLOTTER::PlotFootprintGraphicItem( const FP_SHAPE* aShape )
{
    ...
    default: wxASSERT_MSG( false, "Unhandled FP_SHAPE shape" ); break;
    }
}
    ...
    case S_POLYGON:
        if( aShape->IsPolyShapeValid() )
        {
            if( sketch || thickness > 0 )
            {
                ...
                {
                    auto seg = it.Get();
                    m_plotter->ThickSegment( wxPoint( seg.A ), wxPoint( seg.B ), thickness,
                                             GetPlotMode(), &gbr_metadata );
                }
            }
    ...
    for( TRACK* tracks : m_board->Tracks() )
    {
        ...
        if( via )
        {
            plotOneDrillMark( PAD_DRILL_SHAPE_CIRCLE, via->GetStart(),
                              wxSize( via->GetDrillValue(), 0 ), wxSize( via->GetWidth(), 0 ), 0,
                              smallDrill );
        }
    }
