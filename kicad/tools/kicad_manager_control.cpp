...
int KICAD_MANAGER_CONTROL::ShowDesignBlockLibTable( const TOOL_EVENT& aEvent )
{
    ...
    //std::string payload = "";
    //aKiway->ExpressMail( FRAME_DESIGN_BLOCK_EDITOR, MAIL_RELOAD_LIB, payload );
    //aKiway->ExpressMail( FRAME_DESIGN_BLOCK_VIEWER, MAIL_RELOAD_LIB, payload );
...
int KICAD_MANAGER_CONTROL::ShowPluginManager( const TOOL_EVENT& aEvent )
{
        ...
        && ( settings->m_PcmLibAutoAdd || settings->m_PcmLibAutoRemove ) )
    {
        ...
        Prj().SetElem( PROJECT::ELEM_DESIGN_BLOCK_LIB_TABLE, nullptr );
...
void KICAD_MANAGER_CONTROL::setTransitions()
{
    ...
    Go( &KICAD_MANAGER_CONTROL::ShowDesignBlockLibTable,
                                                   KICAD_MANAGER_ACTIONS::showDesignBlockLibTable.MakeEvent() );
