...
int LAYERS_MAP_DIALOG:: findNumAltiumGerbersLoaded(std::vector<int> &aGerber2KicadMapping)
{
    ...
    wxString AltiumExt[] = { "GTL", // Top copper
                             ...
                             "GBL", // Bottom copper
                             "", "", //Adhesive layers = no assignment
							 "GBP", "GTP", // Paste bottom/top
							 "GBO", "GTO", // Silk Bottom/top
							 "GBS", "GTS", // solder mask Bottom/top
							 "", "", // dwgs & cmts user = no assignment
							 "GM1", "GM2", // "Eco1_user" & "Eco2_user" = Altium mechanical layers
							 "GKO" // PCB Outline
							 };
    int				NumExts = sizeof(AltiumExt) / sizeof(AltiumExt[0]);
	...
	for( int ii = 0; ii < m_gerberActiveLayersCount; ii++ )
	    {
	        ...
	        //DbgMsg = DbgMsg + images->GetGbrImage( ii )->m_FileName;
	        //DbgMsg = DbgMsg + fn.GetExt() + " "; // Build debug string of file extensions
	        //label = new wxStaticText( this, wxID_STATIC, fn.GetFullName() );

        	bool AltiumMatched = false;
        	int jj = 0;
	        while (jj < NumExts)
	        {
	        	AltiumMatched = false;
	        	if (0 == AltiumExt[jj].CmpNoCase(fn.GetExt() )) // Does current layer's Gerber have an Altium extension?
	        	{
	        		// Yes.  Found an Altium Gerber file extension
	        		aGerber2KicadMapping.push_back(jj); //
	        		AltiumMatched = true;
	        		NumProtelMatches++;
					// Add to debug string to show the match we found on this layer
	        		DbgMsg = DbgMsg + "Matched Gerber layer " + wxString::Format(wxT("%i"), ii + 1);
	        		DbgMsg = DbgMsg + " to KiCad layer " + GetPCBDefaultLayerName( jj ) + " (" + wxString::Format(wxT("%i"), jj) + ")\n";
	        		break;
	        	}
	        	jj++;
	        }

	        // If there's no Altium match, then note the layer as undefined
	        if (!AltiumMatched)
	        	aGerber2KicadMapping.push_back(UNDEFINED_LAYER);
