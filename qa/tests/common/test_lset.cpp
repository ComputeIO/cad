...
// Test LSET::operator==
BOOST_AUTO_TEST_CASE( LSETEqOperator )
{
    LSET lset1( { F_Cu, B_Cu, In1_Cu } );
    LSET lset2( { F_Cu, B_Cu, In1_Cu } );
    LSET lset3( { F_Cu, B_Cu } );

    BOOST_CHECK( lset1 == lset2 );
    BOOST_CHECK( !(lset1 == lset3) );
}

// Test LSET::operator!=
BOOST_AUTO_TEST_CASE( LSETNeqOperator )
{
    LSET lset1( { F_Cu, B_Cu, In1_Cu } );
    LSET lset2( { F_Cu, B_Cu, In1_Cu } );
    LSET lset3( { F_Cu, B_Cu } );
...
// Test LSET::operator&
BOOST_AUTO_TEST_CASE( LSETAndOperator )
{
    LSET lset1( { F_Cu, B_Cu, In1_Cu } );
    LSET lset2( { F_Cu, In1_Cu } );
    LSET expected( { F_Cu, In1_Cu } );

    LSET result = lset1 & lset2;
    BOOST_CHECK( result == expected );
}

// Test LSET::operator|
BOOST_AUTO_TEST_CASE( LSETOrOperator )
{
    LSET lset1( { F_Cu, B_Cu } );
    LSET lset2( { In1_Cu, In2_Cu } );
    LSET expected( { F_Cu, B_Cu, In1_Cu, In2_Cu } );
...
// Test LSET::operator^
BOOST_AUTO_TEST_CASE( LSETXorOperator )
{
    LSET lset1( { F_Cu, B_Cu, In1_Cu } );
    LSET lset2( { B_Cu, In1_Cu, In2_Cu } );
    LSET expected( { F_Cu, In2_Cu } );

    LSET result = lset1 ^ lset2;
    BOOST_CHECK( result == expected );
}

// Test LSET boolean operators with different sizes
BOOST_AUTO_TEST_CASE( LSETBooleanOperatorsDifferentSizes )
{
    LSET lset1( { F_Cu, B_Cu, In1_Cu } );
    LSET lset2( { F_Cu, B_Cu, In1_Cu, In2_Cu, B_SilkS } );
    ...
    // Test operator&
    LSET expectedAnd( { F_Cu, B_Cu, In1_Cu } );
    LSET resultAnd = lset1 & lset2;
    BOOST_CHECK( resultAnd == expectedAnd );

    // Test operator|
    LSET expectedOr( { F_Cu, B_Cu, In1_Cu, In2_Cu, B_SilkS } );
    LSET resultOr = lset1 | lset2;
    BOOST_CHECK( resultOr == expectedOr );

    // Test operator^
    LSET expectedXor( { In2_Cu, B_SilkS } );
    LSET resultXor = lset1 ^ lset2;
    BOOST_CHECK( resultXor == expectedXor );
